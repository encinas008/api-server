...
class Authorization_Token 
{
     ...
     * ( 1 Hour ) : 60 * 60     = 3600
     */
    protected $token_expire_time = TOKEN_EXPIRE_TIME_DAY; 
     ...
     * @param token  este valor se ha agregado debido a que en react no envia el header authorization
    ...
    public function validateToken($token = "")
    {
        ...
        /**  solo por el caso de react que no puede enviar el token */
        if($token_data['status'] === FALSE && !empty($token)){
            $token_data = ['status' => TRUE, 'key' => 'Authorization'];
